/**
 * Harem Altın Özelleştirme Çekirdek Modülü (harem_core.js)
 * v2.2 - Converter Options Fix & Better Scaling
 */

(function () {
    console.log('HaremCore v2.2 Başlatılıyor...');
    ...
    function loadConfig() {
        ...
        try {
            ...
            return {
                ...
                converterOptions: { ...DEFAULT_CONFIG.converterOptions, ...parsed.converterOptions }
            };
        } catch (e) {
    ...
    function applyChanges() {
        ...
        applyVisibility();
        applyPriceOffsets();
        applyConverterOptions();
    }

    // Seçiciyi daha sağlam bulmak için
    function getConverterSelect() {
        // Öncelik 1: .translate select
        let sel = document.querySelector('.translate select');
        if (sel) return sel;

        // Öncelik 2: Başlığında "Çevirici" yazan kutuyu bul, onun içinde select ara
        const boxes = document.querySelectorAll('.box');
        for (const box of boxes) {
            const title = box.querySelector('.head .title');
            if (title && title.innerText.trim().includes('Çevirici')) {
    ...
    function applyVisibility() {
        ...
        boxes.forEach(box => {
            ...
            if (sectionName && config.visibility.hasOwnProperty(sectionName)) {
                box.style.display = config.visibility[sectionName] ? '' : 'none';
            }
        });
    }

    function applyConverterOptions() {
        const select = getConverterSelect();
        if (!select) return;

        const options = select.querySelectorAll('option');
        options.forEach(opt => {
            const val = opt.value;
            const isVisible = config.converterOptions.hasOwnProperty(val) ? config.converterOptions[val] : true;
    ...
    function createAdminPanel() {
            ...
            <div id="harem-admin-modal">
                ...
                <div class="admin-body">
                    ...
                    <div style="margin-bottom: 20px; border-top: 1px solid #333; padding-top: 10px;">
                        <h4 style="color: #FFD700; margin-bottom: 10px;">Çevirici Seçenekleri</h4>
                        <div id="admin-converter-options-list">
                            <i style="color:#888; font-size:12px">Çevirici bulunamadı veya yükleniyor...</i>
                        </div>
                    </div>
        ...
        document.getElementById('admin-reset-btn').addEventListener('click', () => {
            if (confirm('Tüm ayarları silip varsayılana dönmek istiyor musunuz?')) {
    ...
    function loadAdminValues() {
        ...
        // --- Çevirici Seçenekleri Doldurma ---
        const converterList = document.getElementById('admin-converter-options-list');
        const select = getConverterSelect();
        ...
        if (select) {
            ...
            options.forEach(opt => {
                ...
                const isChecked = config.converterOptions.hasOwnProperty(val) ? config.converterOptions[val] : true;

                const div = document.createElement('div');
                div.className = 'checkbox-group';
                div.innerHTML = `
                    <input type="checkbox" id="conv-opt-${val}" data-value="${val}" ${isChecked ? 'checked' : ''}>
                    <label for="conv-opt-${val}">${text}</label>
                `;
                converterList.appendChild(div);
            });
        } else {
            converterList.innerHTML = '<i style="color:#888;">Çevirici bulunamadı (Sayfa yükleniyor olabilir, tekrar deneyin).</i>';
        }


        // --- Fiyat Listesi Doldurma ---
        ...
        rows.forEach(row => {
            ...
            item.innerHTML = `
                ...
                <div class="offset-group">
                    <div style="flex:1">
                        <span class="form-label">Alış Farkı</span>
                        <input type="number" class="buy-offset" value="${conf.buyOffset}" step="0.01">
                    </div>
                    <div style="flex:1">
                        <span class="form-label">Satış Farkı</span>
                         <input type="number" class="sell-offset" value="${conf.sellOffset}" step="0.01">
                    </div>
                </div>
            `;
    ...
    function saveConfigFromUI() {
        ...
        // Converter Options
        document.querySelectorAll('#admin-converter-options-list input[type="checkbox"]').forEach(chk => {
            const val = chk.dataset.value;
            config.converterOptions[val] = chk.checked;
        });
